...
QInterfacePtr QBdt::MakeQInterface(bitLenInt qbCount)
{
    return CreateQuantumInterface(engines, qbCount, 0U, rand_generator, ONE_CMPLX, doNormalize, randGlobalPhase, false,
        devID, hardware_rand_generator != NULL, false, (real1_f)amplitudeFloor);
}
    ...
    const complex* mtrx, const bitLenInt* controls, bitLenInt controlLen, bitLenInt target, bool isAnti)
{
    ...
    for (bitCapInt i = 0U; i < gatePower; i++) {
        QBdtNodeInterfacePtr leaf = root;
        QBdtNodeInterfacePtr parent = NULL;
        for (bitLenInt j = 0U; j < gateQubitCount; j++) {
...
bitCapInt QBdt::MAll()
{
    ...
    for (bitLenInt i = 0U; i < qubitCount; i++) {
        ...
        const real1_f oneChance = clampProb(norm(leaf->branches[1]->scale));
        bool bitResult;
        if (oneChance >= ONE_R1) {
            bitResult = true;
        } else if (oneChance <= ZERO_R1) {
    ...
    const complex* mtrx, const bitLenInt* controls, bitLenInt controlLen, bitLenInt target, bool isAnti)
{
    ...
    for (bitLenInt i = 0U; isFallback && (i < controlLen); i++) {
        if (controls[i] != i) {
            isFallback = false;
        }
    }
